...
export class QueueService extends WorkerHost {
  private script = join(process.env['scriptDir'] || join('..', '..', '..', 'python'), process.env['scriptFile'] || 'test.py');
  ...
  async process(bullJob: BullJob<RedisJob>) {
    ...
    bullJob.updateData({ ...bullJob.data, task: { ...bullJob.data.task, results: [result] } });
  ...
  async findJobs(sessionId: UUID, types?: JobType[]) {
    ...
    return Promise.all(bullJobs.filter((bullJob) => bullJob.data.task.sessionId === sessionId).map(async (bullJob) => await this.toDTO(bullJob)));
  }
  ...
  async deleteJobByFilename(sessionId: UUID, filename: string) {
    const bullJob = (await this.jobQueue.getJobs())
      ...
      .find((bullJob) => bullJob.data.task.results.find((result: TaskResult) => result.filename === filename));
    if (!!bullJob) {
  ...
  private async findJobsOfTask(taskId: UUID, jobIdCompleted?: UUID) {
    return await Promise.all(
      (await this.jobQueue.getJobs())
        .filter((bullJob) => bullJob.data.task.id === taskId)
        .map(
          async (bullJob: BullJob<RedisJob>) =>
            ({
              jobId: bullJob.id,
